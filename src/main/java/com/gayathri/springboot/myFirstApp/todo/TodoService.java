package com.gayathri.springboot.myFirstApp.todo;

import java.time.LocalDate;
import java.util.ArrayList;
import java.util.List;
import java.util.function.Predicate;

import org.springframework.stereotype.Service;

import jakarta.validation.Valid;



@Service
public class TodoService {
	private static List<Todo> todos=new ArrayList();
	private static int todosCount=0;
	static {
//		todos.add(new Todo(++todosCount,"gayathri","learn spring",
//				LocalDate.now().plusWeeks(1),false));
//		todos.add(new Todo(++todosCount,"gayathri","learn react",
//				LocalDate.now().plusWeeks(4),false));
//		
//		todos.add(new Todo(++todosCount,"gayathri","learn struts",
//				LocalDate.now().plusWeeks(2),false));
		
	}
	
	public List<Todo> findByUsername(String username){
		Predicate<? super Todo> predicate=todo -> todo.getUsername().equalsIgnoreCase(username);
		return todos.stream().filter(predicate).toList();
	}
	public void addTodo(String username,String desc, LocalDate targetDate, boolean done) {
		Todo todo=new Todo(++todosCount,username,desc,targetDate,done);
		todos.add(todo);
	}
	/*todoController*/
	
	public void deletById(int id) {
		//predicate is a condition
		//todo.getId()==id then delete
		//todo -> todo.getId()==id
		Predicate<? super Todo> predicate=todo -> todo.getId()==id;
//		if(todos.removeIf(predicate)) {
//			--todosCount;
//		}
		todos.removeIf(predicate);
		
	}

	public Todo findById(int id) {
		// TODO Auto-generated method stub
		Predicate<? super Todo> predicate=todo -> todo.getId()==id;
		Todo todo=todos.stream().filter(predicate).findFirst().get();
		return todo;
	}

	public void updateTodo(@Valid Todo todo) {
		// TODO Auto-generated method stub
		deletById(todo.getId());
		todos.add(todo);
	}
	
}
